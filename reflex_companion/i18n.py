...
UI = {
    "en": {
        ...
        # v0.19.51 — Capabilities banner per model
        "caps_banner_title":  "What this model can do for Ashley",
        "caps_vision_yes":    "✅ Vision (Ashley can see your screen)",
        "caps_vision_no":     "❌ Vision NOT supported — toggle 👁 won't work",
        "caps_actions_high":  "✅ Actions (open apps, control PC reliably)",
        "caps_actions_ok":    "🟡 Actions (mostly works, occasional misses)",
        "caps_actions_low":   "⚠️ Actions unreliable (small model — may ignore tags)",
        "caps_websearch_yes": "✅ Discovery / web search (proactive content)",
        "caps_websearch_no":  "❌ Discovery / web search not available with this provider",
        "caps_quality_label": "Companion quality:",
        "caps_warning_title": "Tips for choosing a model:",
        "caps_warning_xai":   "Grok = best all-around. All features work, you pay xAI per token.",
        "caps_warning_or":    "OpenRouter = premium models (Claude/GPT). Loses Discovery (no web search). DeepSeek/MiniMax also lose Vision.",
        "caps_warning_oll":   "Ollama = 100% free local. Most models text-only — for Vision use llava or llama3.2-vision. Small models (3B) may ignore actions.",
    ...
    "es": {
        ...
        # v0.19.51 — Banner de capabilities por modelo
        "caps_banner_title":  "Qué puede hacer este modelo con Ashley",
        "caps_vision_yes":    "✅ Visión (Ashley puede ver tu pantalla)",
        "caps_vision_no":     "❌ Visión NO soportada — el toggle 👁 no funcionará",
        "caps_actions_high":  "✅ Acciones (abre apps, controla PC bien)",
        "caps_actions_ok":    "🟡 Acciones (funciona casi siempre, fallos ocasionales)",
        "caps_actions_low":   "⚠️ Acciones poco fiables (modelo pequeño — puede ignorar tags)",
        "caps_websearch_yes": "✅ Discovery / búsqueda web (contenido proactivo)",
        "caps_websearch_no":  "❌ Discovery / búsqueda web no disponible con este provider",
        "caps_quality_label": "Calidad como compañera:",
        "caps_warning_title": "Consejos para elegir modelo:",
        "caps_warning_xai":   "Grok = la mejor opción global. Todas las features funcionan, pagas a xAI por token.",
        "caps_warning_or":    "OpenRouter = modelos premium (Claude/GPT). Pierdes Discovery (sin web search). DeepSeek/MiniMax también pierden Visión.",
        "caps_warning_oll":   "Ollama = 100% gratis y local. Casi todos los modelos son sólo-texto — para Visión usa llava o llama3.2-vision. Modelos pequeños (3B) pueden ignorar acciones.",
    ...
    "fr": {
        ...
        # v0.19.51 — Bannière capacités par modèle
        "caps_banner_title":  "Ce que ce modèle peut faire avec Ashley",
        "caps_vision_yes":    "✅ Vision (Ashley peut voir ton écran)",
        "caps_vision_no":     "❌ Vision NON supportée — le bouton 👁 ne fonctionnera pas",
        "caps_actions_high":  "✅ Actions (ouvre apps, contrôle PC bien)",
        "caps_actions_ok":    "🟡 Actions (marche presque toujours, ratés occasionnels)",
        "caps_actions_low":   "⚠️ Actions peu fiables (petit modèle — peut ignorer les tags)",
        "caps_websearch_yes": "✅ Discovery / recherche web (contenu proactif)",
        "caps_websearch_no":  "❌ Discovery / recherche web non disponible avec ce provider",
        "caps_quality_label": "Qualité comme compagne :",
        "caps_warning_title": "Conseils pour choisir un modèle :",
        "caps_warning_xai":   "Grok = la meilleure option globale. Toutes les features marchent, tu paies xAI au token.",
        "caps_warning_or":    "OpenRouter = modèles premium (Claude/GPT). Tu perds Discovery (pas de web search). DeepSeek/MiniMax perdent aussi Vision.",
        "caps_warning_oll":   "Ollama = 100% gratuit et local. La plupart des modèles sont texte-seul — pour Vision utilise llava ou llama3.2-vision. Petits modèles (3B) peuvent ignorer actions.",
    ...
    "ja": {
        ...
        # v0.19.51 — モデルごとの機能バナー
        "caps_banner_title":  "このモデルで Ashley ができること",
        "caps_vision_yes":    "✅ ビジョン (Ashleyは画面を見える)",
        "caps_vision_no":     "❌ ビジョン非対応 — 👁 ボタンは機能しない",
        "caps_actions_high":  "✅ アクション (アプリを開く、PC制御が確実)",
        "caps_actions_ok":    "🟡 アクション (ほぼ機能、たまにミス)",
        "caps_actions_low":   "⚠️ アクション不安定 (小型モデル — タグを無視する可能性)",
        "caps_websearch_yes": "✅ Discovery / ウェブ検索 (能動的なコンテンツ)",
        "caps_websearch_no":  "❌ Discovery / ウェブ検索はこのプロバイダで利用不可",
        "caps_quality_label": "コンパニオン品質:",
        "caps_warning_title": "モデル選びのヒント:",
        "caps_warning_xai":   "Grok = 全体的に最良。全機能動作、xAI にトークン課金。",
        "caps_warning_or":    "OpenRouter = プレミアムモデル (Claude/GPT)。Discovery は失う (ウェブ検索なし)。DeepSeek/MiniMax は Vision も失う。",
        "caps_warning_oll":   "Ollama = 100% 無料ローカル。多くのモデルがテキストのみ — Vision には llava か llama3.2-vision を使う。小型モデル (3B) はアクションを無視する可能性。",
    ...
    "de": {
        ...
        # v0.19.51 — Capabilities-Banner pro Modell
        "caps_banner_title":  "Was dieses Modell mit Ashley kann",
        "caps_vision_yes":    "✅ Vision (Ashley sieht deinen Bildschirm)",
        "caps_vision_no":     "❌ Vision NICHT unterstützt — der 👁-Knopf wird nicht funktionieren",
        "caps_actions_high":  "✅ Aktionen (öffnet Apps, steuert PC zuverlässig)",
        "caps_actions_ok":    "🟡 Aktionen (meist OK, gelegentliche Aussetzer)",
        "caps_actions_low":   "⚠️ Aktionen unzuverlässig (kleines Modell — kann Tags ignorieren)",
        "caps_websearch_yes": "✅ Discovery / Websuche (proaktiver Inhalt)",
        "caps_websearch_no":  "❌ Discovery / Websuche nicht mit diesem Provider verfügbar",
        "caps_quality_label": "Companion-Qualität:",
        "caps_warning_title": "Tipps zur Modellwahl:",
        "caps_warning_xai":   "Grok = beste Allround-Option. Alle Features funktionieren, du zahlst xAI pro Token.",
        "caps_warning_or":    "OpenRouter = Premium-Modelle (Claude/GPT). Du verlierst Discovery (keine Websuche). DeepSeek/MiniMax verlieren auch Vision.",
        "caps_warning_oll":   "Ollama = 100% kostenlos lokal. Die meisten Modelle nur Text — für Vision nutze llava oder llama3.2-vision. Kleine Modelle (3B) können Aktionen ignorieren.",
    ...
    "ru": {
        ...
        # v0.19.51 — Баннер возможностей по модели
        "caps_banner_title":  "Что эта модель может с Ashley",
        "caps_vision_yes":    "✅ Зрение (Ashley видит твой экран)",
        "caps_vision_no":     "❌ Зрение НЕ поддерживается — кнопка 👁 не сработает",
        "caps_actions_high":  "✅ Действия (открывает приложения, надёжно управляет ПК)",
        "caps_actions_ok":    "🟡 Действия (в основном работает, иногда промахи)",
        "caps_actions_low":   "⚠️ Действия ненадёжны (маленькая модель — может игнорировать теги)",
        "caps_websearch_yes": "✅ Discovery / веб-поиск (проактивный контент)",
        "caps_websearch_no":  "❌ Discovery / веб-поиск недоступен с этим провайдером",
        "caps_quality_label": "Качество как компаньона:",
        "caps_warning_title": "Советы по выбору модели:",
        "caps_warning_xai":   "Grok = лучший общий вариант. Все функции работают, платишь xAI за токены.",
        "caps_warning_or":    "OpenRouter = премиум модели (Claude/GPT). Теряешь Discovery (нет веб-поиска). DeepSeek/MiniMax также теряют Зрение.",
        "caps_warning_oll":   "Ollama = 100% бесплатно локально. Большинство моделей только текст — для Зрения используй llava или llama3.2-vision. Маленькие модели (3B) могут игнорировать действия.",
    ...
    "ko": {
        ...
        # v0.19.51 — 모델별 기능 배너
        "caps_banner_title":  "이 모델이 Ashley로 할 수 있는 것",
        "caps_vision_yes":    "✅ 비전 (Ashley가 화면을 볼 수 있어)",
        "caps_vision_no":     "❌ 비전 미지원 — 👁 버튼이 작동 안 해",
        "caps_actions_high":  "✅ 액션 (앱 열기, PC 제어 안정적)",
        "caps_actions_ok":    "🟡 액션 (대부분 작동, 가끔 놓침)",
        "caps_actions_low":   "⚠️ 액션 불안정 (작은 모델 — 태그 무시할 수 있어)",
        "caps_websearch_yes": "✅ Discovery / 웹 검색 (능동적 콘텐츠)",
        "caps_websearch_no":  "❌ Discovery / 웹 검색은 이 프로바이더에서 사용 불가",
        "caps_quality_label": "동반자 품질:",
        "caps_warning_title": "모델 선택 팁:",
        "caps_warning_xai":   "Grok = 전반적으로 최고. 모든 기능 작동, xAI에 토큰당 지불.",
        "caps_warning_or":    "OpenRouter = 프리미엄 모델 (Claude/GPT). Discovery 잃음 (웹 검색 없음). DeepSeek/MiniMax는 비전도 잃음.",
        "caps_warning_oll":   "Ollama = 100% 무료 로컬. 대부분 모델은 텍스트 전용 — 비전은 llava 또는 llama3.2-vision 써. 작은 모델 (3B)은 액션 무시할 수 있어.",
